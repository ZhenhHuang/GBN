...
class BoundaryConvLayer(nn.Module):
    def __init__(self, in_dim, out_dim, bias, act=None, drop=0.3):
        super().__init__()
...
class BoundaryGCN(nn.Module):
    def __init__(self, n_layers, in_dim, embed_dim, out_dim, bias, act, drop=0.3):
        super().__init__()
        self.layers = nn.ModuleList([BoundaryConvLayer(in_dim, embed_dim, bias, act, drop)])
        self.res_lin = nn.ModuleList([nn.Linear(in_dim, embed_dim, bias=False) for _ in range(n_layers - 1)])
        for _ in range(n_layers - 2):
            self.layers.append(BoundaryConvLayer(embed_dim, embed_dim, bias, act, drop))
